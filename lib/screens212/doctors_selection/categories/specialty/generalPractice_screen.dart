...
class GenreralParcticeScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorBySpecialtyCubit()..showgeneralPracticeDoctors(),
        child: BlocConsumer<GetDoctorBySpecialtyCubit,ShowDoctorsBySpecialtyStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorBySpecialtyCubit.get(context).generalPractice;
            List doctorsId = GetDoctorBySpecialtyCubit.get(context).generalPracticeId;
            return Padding(
