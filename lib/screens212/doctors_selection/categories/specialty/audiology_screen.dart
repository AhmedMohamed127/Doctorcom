...
class AudiologyScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) =>
            GetDoctorBySpecialtyCubit()..showAudiologyDoctors(),
        child: BlocConsumer<GetDoctorBySpecialtyCubit,
            ShowDoctorsBySpecialtyStates>(
          listener: (context, state) {},
          builder: (context, state) {
            List doctors =
                GetDoctorBySpecialtyCubit.get(context).audiologyDoctors;
            List doctorsId =
                GetDoctorBySpecialtyCubit.get(context).audiologyDoctorsId;
            return Padding(
              padding: const EdgeInsets.all(1.0),
              child: ListView.separated(
                itemBuilder: (context, index) => Shape(
                  ...
                  specialization:
                      'specialized in ${doctors[index]['doctorSpecialty']}',
                  fees: 160,
