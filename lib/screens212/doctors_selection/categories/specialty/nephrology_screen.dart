...
class NaphrologyScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorBySpecialtyCubit()..shownephrologyDoctors(),
        child: BlocConsumer<GetDoctorBySpecialtyCubit,ShowDoctorsBySpecialtyStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorBySpecialtyCubit.get(context).nephrologyDoctors;
            List doctorsId = GetDoctorBySpecialtyCubit.get(context).nephrologyDoctorsId;
            return Padding(
