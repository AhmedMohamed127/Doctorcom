...
class QualubiaScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorByAreaCubit()..showQualubiaDoctors(),
        child: BlocConsumer<GetDoctorByAreaCubit,ShowDoctorsByAreaStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorByAreaCubit.get(context).QualubiaDoctors;
            List doctorsId = GetDoctorByAreaCubit.get(context).QualubiaDoctorsId;
            return Padding(
