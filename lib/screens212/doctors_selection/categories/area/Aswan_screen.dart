...
class AswanScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorByAreaCubit()..showAswanDoctors(),
        child: BlocConsumer<GetDoctorByAreaCubit,ShowDoctorsByAreaStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorByAreaCubit.get(context).AswanDoctors;
            List doctorsId = GetDoctorByAreaCubit.get(context).AswanDoctorsId;
            return Padding(
