...
class MonufiaScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorByAreaCubit()..showMonufiaDoctors(),
        child: BlocConsumer<GetDoctorByAreaCubit,ShowDoctorsByAreaStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorByAreaCubit.get(context).MonufiaDoctors;
            List doctorsId = GetDoctorByAreaCubit.get(context).MonufiaDoctorsId;
            return Padding(
