...
class CairoScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocProvider(
        create: (context)=> GetDoctorByAreaCubit()..showCairoDoctors(),
        child: BlocConsumer<GetDoctorByAreaCubit,ShowDoctorsByAreaStates>(
          listener: (context,state){},
          builder: (context,state){
            List doctors = GetDoctorByAreaCubit.get(context).cairoDoctors;
            List doctorsId = GetDoctorByAreaCubit.get(context).cairoDoctorsId;
            return Padding(
